...
class PixCreateCharge extends StatefulWidget {
  @override
  _PixCreateChargeState createState() => _PixCreateChargeState();
}

class _PixCreateChargeState extends State<PixCreateCharge> {
          ...
          children: [
            _headerForm(
                ...
                "O campo txid, obrigatório, determina o identificador da transação. "
                    "O objetivo desse campo é ser um elemento que possibilite ao "
                    "PSP do recebedor apresentar ao usuário recebedor a funcionalidade "
                    "de conciliação de pagamentos."),
            _form(_formTxId()),
            _headerForm(
                "Calendário",
  ...
  Widget _formTxId() {
    return Row(
      children: [
        Expanded(
            child: FormDataField(
          helperText: "Identificador da transação",
          label: "Txid*",
          hintText: "Ex: PFdJj5fqRnShVolVjzzJTRioFmH",
          line: 1,
        ...
        IconButton(
            icon: Icon(Icons.add, color: Color(0xFF00b4c5)),
            onPressed: _generateTxId),
      ],
    );
  }
  ...
  Widget _appBar() {
    return AppBar(
      elevation: 0,
      title: Text("Criar Cobrança Pix"),
      centerTitle: true,
      actions: [
        _buttonInfo(
            "Criar Cobrança",
            "Endpoint para criar uma cobrança PUT /v2/cob/{txid}.\n\n",
            Colors.white)
      ],
    );
  }
  ...
  void _generateTxId() {
    ...
    _txIdController.text = String.fromCharCodes(Iterable.generate(
        27, (_) => _chars.codeUnitAt(_rnd.nextInt(_chars.length))));
  }

  dynamic _pixCreateCharge() async {
    setState(() => _loading = true);
    Map<String, dynamic> params = {"txid": _txIdController.text};
    ...
    gn.call('pixCreateCharge', params: params, body: body).then((value) {
