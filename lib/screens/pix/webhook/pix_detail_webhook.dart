...
class PixDetailWebhook extends StatefulWidget {
  @override
  _PixDetailWebhookState createState() => _PixDetailWebhookState();
}

class _PixDetailWebhookState extends State<PixDetailWebhook> {
    ...
    return Row(
      children: [
        ...
        IconButton(
            ...
            onPressed: () {
              Navigator.pushNamed(context, "pix/webhook/list");
            }),
      ],
    );
  }
  ...
  Widget _appBar() {
    return AppBar(
      elevation: 0,
      title: Text("Detalhe Webhook"),
      centerTitle: true,
      actions: [
        _buttonInfo(
            "Exibir informações acerca do Wehook Pix",
            "Endpoint para recuperação de informações sobre o webhook pix GET ​/v2/webhook/:chave.\n\n",
            Colors.white)
      ],
    );
  }

  void _pixDetailWebhook() {
    setState(() => _loading = true);
    Map<String, dynamic> params = {
      "chave": _keyController.text,
    };
    gn.call("pixDetailWebhook", params: params).then((value) {
    ...
    showModalBottomSheet<void>(
        ...
        builder: (BuildContext context) {
          return Scaffold(
              appBar: AppBar(
                elevation: 0,
                title: Text(value['chave'], style: TextStyle(fontSize: 15)),
                centerTitle: true,
                leading: IconButton(
              ...
              body: SingleChildScrollView(
                child: Column(
                  children: [
                    _infoCharge("${value['webhookUrl']}", "URL", true),
                    _infoCharge("${value['chave']}", "CHAVE", true),
                    _infoCharge("${value['criacao']}", "CRIAÇÃO", true),
                  ],
                ),
              ));
        });
