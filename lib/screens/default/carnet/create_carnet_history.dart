...
class CreateCarnetHistory extends StatefulWidget {
  @override
  _CreateCarnetHistoryState createState() => _CreateCarnetHistoryState();
}

class _CreateCarnetHistoryState extends State<CreateCarnetHistory> {
    ...
    return SingleChildScrollView(
        child: Form(
            ...
            child: Column(
              children: [
                _headerForm("Carnê",
                    "O campo ID é referente ao ID de um carnê criado anteriormente."),
                _form(_formId()),
                _headerForm("Descrição",
                    "O campo abaixo é referente ao texto a ser acrescentado no histórico do carnê."),
                _form(_formText()),
              ],
            )));
  }
  ...
  Widget _formText() {
    return FormDataField(
      helperText: "Texto a ser acrescentado ao histórico",
      label: "Texto*",
      line: 3,
  ...
  Widget _appBar() {
    return AppBar(
      elevation: 0,
      title: Text("Acrescenta Histórico"),
      centerTitle: true,
      actions: [
        _buttonInfo(
            "Acrescenta Histórico",
            "Acrescentar informações ao histórico de um carnê.\n\n",
            Colors.white)
      ],
    );
  }

  void _createCarnetHistory() {
    ...
    gn.call("createCarnetHistory", params: params, body: body).then((value) {
      setState(() => _loading = false);
      _showMessage("Descrição Acrescentada!", Theme.of(context).accentColor,
