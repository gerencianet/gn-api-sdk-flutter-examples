...
class _CreateSubscriptionState extends State<CreateSubscription> {
      ...
      child: Form(
          ...
          child: Column(
            children: [
              _headerForm(
                  "Plano",
                  "O campo plano ID refere-se ao ID de um plano criado anteriormente.\n"
                      "Você pode consultar os planos cadastrados clicando no icone na frente do campo Plano ID."),
              _form(_formPlanId()),
              _headerForm("Item",
                  "Os campos abaixo são referentes ao item a ser associado na assinatura.\n"),
              _form(_formName()),
              _form(_formAmount()),
              _form(_formValue()),
  ...
  Widget _formPlanId() {
    return Row(
      children: [
        Expanded(
            child: FormDataField(
          helperText: "ID do plano a ser associado a assinatura",
          label: "Plano ID*",
          hintText: "ex: 1234",
          line: 1,
  ...
  void _createSubscription() {
    ...
    gn.call("createSubscription", params: params, body: body).then((value) {
      setState(() => _loading = false);
      _showMessage("Assinatura Cadastrada!", Theme.of(context).accentColor,
